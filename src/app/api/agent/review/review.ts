...
export async function POST(request: NextRequest) {
  try {
    ...
    const extractionContext = extractionChunks
      .map((c, i) => `[Section ${i + 1}]\n${c.content}`)
      .join('\n\n')
...
Format:
{
  ...
  "recommendations": ["string"]
    ...
    const summaryContext = summaryChunks
      .map((c, i) => `[Section ${i + 1}]\n${c.content}`)
      .join('\n\n')
...
Format:
{
  ...
  "key_points": ["string"]
    ...
    const reportResponse = await generateCompletion(
      `You are Veridoc, an enterprise contract review agent.
You have completed a full automated review of the contract.
Write a professional contract review report conclusion in 2-3 paragraphs.
